...
def atom_to_serialized_bytes (atom : Array UInt8) : Array UInt8 :=
  ...
  if size == 0 then
    ...
    #[(0x80.lor size).toUInt8] ++ atom
  else if size <= 0x2000 then
    #[(0xC0.lor (size.shiftRight 8)).toUInt8, ((size.shiftRight 0).land 0xFF).toUInt8] ++ atom
  else if size <= 0x100000 then
    #[(0xE0.lor (size.shiftRight 16)).toUInt8, ((size.shiftRight 8).land 0xFF).toUInt8, ((size.shiftRight 0).land 0xFF).toUInt8] ++ atom
  else if size <= 0x8000000 then
    #[(0xF0.lor (size.shiftRight 24)).toUInt8, ((size.shiftRight 16).land 0xFF).toUInt8, ((size.shiftRight 8).land 0xFF).toUInt8, ((size.shiftRight 0).land 0xFF).toUInt8] ++ atom
...
def node_to_bytes (node : Node) : Array UInt8 :=
  match node with
...
def bytes_to_atom (bytes: Array UInt8) : Option DResult :=
  match bytes[0]? with
  | none => none
  | some o =>
      if o = 0x80 then
        let new_bytes := bytes.extract 1 bytes.size
        some (DResult.mk new_bytes (Node.atom #[]))
      else
      if o <= MAX_SINGLE_BYTE then
        ...
        if o.land 0xc0 = 0x80 then
          let atom_size := (o.land 0x3f).toNat
          ...
          if o.land 0xe0 = 0xc0 then
            ...
            if bytes.size < atom_start_offset then
              ...
              let atom_size := (o.land 0x1f).toNat.shiftLeft 8 + (bytes[1]!).toNat
              let new_bytes_offset := atom_start_offset + atom_size
              if bytes.size + 1 < new_bytes_offset then
                none
              else
            ...
            if o.land 0xf0 = 0xe0 then
              ...
              if bytes.size < atom_start_offset then
                ...
                let atom_size := (o.land 0xf).toNat.shiftLeft 16 + (bytes[1]!).toNat.shiftLeft 24 + (bytes[2]!).toNat
                let new_bytes_offset := atom_start_offset + atom_size
                if bytes.size + 1 < new_bytes_offset then
                  none
                else
...
def bytes_to_node_inner (heartbeat_count: Nat) (bytes: Array UInt8) : Result DResult :=
  if heartbeat_count = 0 then
    ...
    match bytes[0]? with
    | none => Result.err (DResult.mk bytes (Node.atom #[])) "end of stream"
    | some o =>
      if o = 255 then
...
def bytes_to_node (bytes: Array UInt8) : Node :=
